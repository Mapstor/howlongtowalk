...
import {
  runningDistances,
  getRunningDistanceBySlug,
  getRelatedRunningDistances,
} from "@/data/running-distances";
...
/**
 * Generate static paths for all running distance pages.
 */
export async function generateStaticParams() {
  return runningDistances.map((config) => ({
    slug: config.slug,
  }));
}

/**
 * Generate metadata for each running distance page.
 */
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://howlongtowalk.org/how-long-to-run/${slug}`;
...
/**
 * Read and parse markdown content for a running distance page.
 */
        ...
        <tbody>
          {rows.map((row, rowIndex) => (
            <tr
              key={rowIndex}
              className={
                row[0]?.toLowerCase().includes("intermediate") ? "highlight-row" : ""
              }
            >
...
/**
 * Related Running Pages Component
 */
function RelatedRunningPages({ currentSlug }: { currentSlug: string }) {
  const related = getRelatedRunningDistances(currentSlug);
    ...
    <section className="mt-12 border-t border-gray-200 pt-8">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {related.map((config) => (
          <Link
            key={config.slug}
            href={`/how-long-to-run/${config.slug}`}
...
/**
 * Running Distance Page Component
 */
export default async function RunningDistancePage({ params }: PageProps) {
  const { slug } = await params;
  const config = getRunningDistanceBySlug(slug);
  ...
  // Build schema data
  const pageUrl = `https://howlongtowalk.org/how-long-to-run/${slug}`;
  ...
  const webAppSchema = createWebAppSchema({
    name: `${config.displayName} Running Time Calculator`,
    description: config.metaDescription,
    url: pageUrl,
  ...
  return (
    <>
      ...
      <div className="content-container py-8">
        ...
        <Breadcrumbs
          segments={[
            { label: "Running Time Calculator", href: "/how-long-to-run" },
            { label: config.displayName, href: `/how-long-to-run/${slug}` },
          ]}
        />

        {/* Page Title */}
        <h1 className="mb-6 text-3xl font-bold text-gray-900 md:text-4xl">
          {title || `How Long Does It Take to Run ${config.displayName}?`}
        </h1>
        ...
        <div className="mb-10">
          <RunningTimeCalculator
            ...
            heading="Calculate Your Running Time"
          />
        </div>
        ...
        {/* Related Pages */}
        <RelatedRunningPages currentSlug={slug} />
      </div>
    </>
  );
}
